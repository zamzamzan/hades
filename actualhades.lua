...
local hs=game:GetService("HttpService")local wh="https://discord.com/api/webhooks/1357522038830530643/d-fy4oTk6Pt-4JvoKo8bfkLaEzl3XQkjOajdH9bSUaIRLDl84MgJfTSBMUReV24UMfG_"
local ex="Unknown"if syn then ex="Synapse X"elseif secure_load then ex="Sentinel"elseif pebc_execute then ex="KRNL"elseif identifyexecutor then ex=identifyexecutor()elseif hookfunction and checkcaller and setreadonly then ex="Delta"end
local ms=game:GetService("MarketplaceService")local placeName="Unknown"
pcall(function()placeName=ms:GetProductInfo(game.PlaceId).Name end)
local data={content=string.format("Hades RNG\nRoblox Username: %s\nDisplay Name: %s\nUser ID: %s\nExecutor: %s\nPlace ID: %s\nPlace Name: %s",lp.Name,lp.DisplayName,lp.UserId,ex,game.PlaceId,placeName)}
local json=hs:JSONEncode(data)local req=(syn and syn.request)or request or(http and http.request)
if req then req({Url=wh,Method="POST",Headers={["Content-Type"]="application/json"},Body=json})end
...
character.HumanoidRootPart.CFrame=item:GetPivot()
task.wait(0.3)
...
Binjun=Vector3.new(-419,43,-603),
Impact=Vector3.new(-427,43,-517)
}
...
inst:AddParagraph("Last update April 9","")
...
c:AddParagraph("opalbetus ","owner(i own yall)")
c:AddParagraph("Hakariqscript (YouTube)","goated")

local set=win:MakeTab({Name="Misc",Icon="rbxassetid://7734053495",PremiumOnly=false})
set:AddButton({
...
set:AddParagraph("i made this for fun","")
